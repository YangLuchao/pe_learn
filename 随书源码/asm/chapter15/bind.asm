...
szFileNameOpen2      db 'c:\IEXPLORE.EXE',MAX_PATH dup(0)
...
_openFile proc
  ...
  mov dwPatchCodeSize,eax ;补丁代码大小放入到全局变量中
  ...
  invoke _getRVACount,@lpMemory1;获取节表个数
  inc eax
  xor edx,edx
  ...
  mov dword ptr lpOthers,eax;对齐后的文件头大小放入到全局变量中
  ...
  mov dwNewFileSize,eax;对齐后的新文件大小放入到全局变量中
...
start:
  invoke InitCommonControls;注册并初始化某些常见的控件窗口类,此函数已过时。 新应用程序应使用 InitCommonControlsEx 函数
  invoke LoadLibrary,offset szDllEdit;加载编辑器控件
  mov hRichEdit,eax
  invoke GetModuleHandle,NULL;获取指定进程的句柄，如果参数为NULL，则获取当线程
  mov hInstance,eax
