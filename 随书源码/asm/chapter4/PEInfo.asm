...
_OffsetToRVA proc _lpFileHead,_dwOffset
  ...
  .repeat
    ...
    .if (edi>=[edx].PointerToRawData)&&(edi<eax)
      ...
      ;计算RVA在节中的偏移
      sub edi,eax                
      mov eax,[edx].VirtualAddress;节起始RVA地址
      ;加上节在文件中的的起始位置
      add eax,edi;目标地址RVA = 节起始地址+目标地址在节中的偏移                
      jmp @F
    .endif
    add edx,sizeof IMAGE_SECTION_HEADER
  .untilcxz
...
_getApi proc _hModule,_lpApi
   ...
   xor al,al;al清0
   cld;设置位正向
   repnz scasb;scasb：加载edi所指的字节和al中的字节相减，repnz重复scasb指令，直到0标志符为1
   mov ecx,edi
   sub ecx,_lpApi;计算字符串长度
   mov @dwLen,ecx;将字符串长度挪到局部变量中

   ;从pe文件头的数据目录获取导出表地址
   mov esi,_hModule;动态链接库基址挪到esi中
   add esi,[esi+3ch];+3c到NT头
   assume esi:ptr IMAGE_NT_HEADERS;假设为NT头
   mov esi,[esi].OptionalHeader.DataDirectory.VirtualAddress;导出RVA挪到esi中
   add esi,_hModule;加VA基址
   assume esi:ptr IMAGE_EXPORT_DIRECTORY;假设esi为导出表目录项

   ;查找符合名称的导出函数名
   mov ebx,[esi].AddressOfNames;导出函数名字列表RVA挪到ebx中
   add ebx,_hModule;加基址
   xor edx,edx;edx清空，用于计数
   .repeat
     push esi
     ;----------这一坨就是标准的判断字符串相等的汇编写法
     mov edi,[ebx];名字基址挪到edi中
     add edi,_hModule;修正
     mov esi,_lpApi;目标名字基址挪到esi中
     mov ecx,@dwLen;
     repz cmpsb;repz重复后面的指令，直到cx=0或者zf=0(不相等),cmpsb:esi和edi所指的字节相减，
     .if ZERO?
       pop esi
       jmp @F
     .endif
     ;----------这一坨就是标准的判断字符串相等的汇编写法
     pop esi
     add ebx,4;当前指向的RVA地址不是目标函数名称，+4指向下下一个地址
     inc edx
   .until edx>=[esi].NumberOfNames;直到循环数量大于导出函数数量
   jmp _ret;没有找到，返回了
@@:
   ...
   add ebx,_hModule
   movzx eax,word ptr [ebx];字数据挪到eax中，高位补0
   shl eax,2;逻辑左移，低位补0
