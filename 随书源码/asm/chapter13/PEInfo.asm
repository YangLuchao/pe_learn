...
szNoResource db 0dh,0ah,'未发现该文件有资源表',0dh,0ah,0
szType      db '1-光标         ',0
            db '2-位图         ',0
            db '3-图标         ',0
            db '4-菜单         ',0
...
szOut4      db 0dh,0ah,0dh,0ah,0dh,0ah
            db '---------------------------------------------------------------------------------------------',0dh,0ah
            db '资源表所处的节：%s',0dh,0ah
            db '---------------------------------------------------------------------------------------------',0dh,0ah
            db 0dh,0ah,0dh,0ah
            db '根目录',0dh,0ah
            db '|',0dh,0ah,0

szOut5      db '|-- %d（自定义编号）',0dh,0ah
            db '|   |',0dh,0ah,0

szLevel1    db '|-- %s',0dh,0ah
            db '|   |',0dh,0ah,0
szLevel2    db '|   |-- %s',0dh,0ah
            db '|   |     |',0dh,0ah,0
szOut6      db '|   |-- ID %d',0dh,0ah
            db '|   |     |',0dh,0ah,0
szLevel3    db '|   |     |-- 代码页：%d   资源所在文件位置：0x%08x  资源长度：%d',0dh,0ah,0
...
_getImportInfo proc _lpFile,_lpPeHead,_dwSize
  ...
  .while [edi].OriginalFirstThunk || [edi].TimeDateStamp ||\
    ...
    mov ebx,eax
    .while dword ptr [ebx]
      .break .if dwFlag==1   ;加入一个永远也不可能成立的条件
                             ;该标志会由其他进程修改！！
      ;按序号导入
